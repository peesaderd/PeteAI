...
Generic parts extracted from etsy_mcp so any MCP app can reuse them.
"""
...
def build_tool_module_map(
    ...
    Args:
        ...
        manifest_path: Optional path to a pre-generated ``tools_manifest.json``.
                       ...
                       resolved on disk.
        tool_prefix: Prefix for tool names to discover (default ``"etsy_"``).
                     Use ``"protect_"`` for Protect tools.
                     NOTE: Default changed from ``"unifi_"`` in the unifi-mcp
                     port of this module; callers that want the old behavior
                     should pass ``tool_prefix="unifi_"`` explicitly.
