...
async def register_tools_for_mode(
    *,
    ...
    Args:
        ...
        base_package: Dotted package path for tool modules (e.g. ``"etsy_mcp.tools"``).
        config: Server config object (used for ``enabled_categories``/``enabled_tools``).
        logger: Logger instance.
        prefix: Tool name prefix (e.g. ``"protect"``). Empty string for Network (uses ``"etsy"``).
        server_label: Human-readable server name (e.g. ``"Etsy Protect"``).
    ...
    if register_meta_tools is None:
        from etsy_mcp_shared.meta_tools import register_meta_tools
    if register_load_tools is None:
        from etsy_mcp_shared.meta_tools import register_load_tools
    ...
    if mode == "meta_only":
        ...
        logger.info("   To load all tools directly: set ETSY_TOOL_REGISTRATION_MODE=eager")
