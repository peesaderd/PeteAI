...
Three-level hierarchy (most specific wins):
    ETSY_POLICY_<ACTION>                              - global
    ETSY_POLICY_<SERVER>_<ACTION>                     - per-server
    ETSY_POLICY_<SERVER>_<CATEGORY>_<ACTION>           - per-category

Permission mode controls mutation handling:
    ETSY_TOOL_PERMISSION_MODE=confirm|bypass           - global
    ETSY_<SERVER>_TOOL_PERMISSION_MODE=confirm|bypass  - per-server
"""
...
class PolicyGateChecker:
    ...
    def check(self, category: str, action: str) -> bool:
        ...
        env_vars = [
            f"ETSY_POLICY_{self.server_prefix}_{config_key}_{action_upper}",
            f"ETSY_POLICY_{self.server_prefix}_{action_upper}",
            f"ETSY_POLICY_{action_upper}",
        ]
        ...
        # 4. Backwards compat: old ETSY_PERMISSIONS_ format
        old_var = f"ETSY_PERMISSIONS_{config_key}_{action_upper}"
    ...
    def denial_message(self, category: str, action: str) -> str:
        ...
        config_key = self._resolve_category(category).upper()
        action_upper = action.upper()
        enable_var = f"ETSY_POLICY_{self.server_prefix}_{config_key}_{action_upper}"
        return (
...
def resolve_permission_mode(server_prefix: str) -> str:
    ...
    Priority: server-specific > global > ETSY_AUTO_CONFIRM compat > default.
    """
    prefix_upper = server_prefix.upper()

    # 1. Server-specific mode
    server_var = f"ETSY_{prefix_upper}_TOOL_PERMISSION_MODE"
    ...
    # 2. Global mode
    global_val = os.environ.get("ETSY_TOOL_PERMISSION_MODE")
    if global_val and global_val.strip().lower() in VALID_PERMISSION_MODES:
        return global_val.strip().lower()

    # 3. Backwards compat: ETSY_AUTO_CONFIRM=true -> bypass
    auto_confirm = os.environ.get("ETSY_AUTO_CONFIRM", "").strip().lower()
    if auto_confirm in _TRUTHY:
        logger.warning(
            "[permissions] ETSY_AUTO_CONFIRM is deprecated. "
            "Use ETSY_TOOL_PERMISSION_MODE=bypass instead."
        )
        return "bypass"
...
def check_deprecated_env_vars(server_prefix: str, logger) -> None:
    ...
    old_prefix = "ETSY_PERMISSIONS_"
    prefix_upper = server_prefix.upper()
    for key, value in os.environ.items():
        if key.startswith(old_prefix):
            category_action = key[len(old_prefix):]
            new_key = f"ETSY_POLICY_{prefix_upper}_{category_action}"
            logger.warning(
    ...
    # Also check ETSY_AUTO_CONFIRM
    if os.environ.get("ETSY_AUTO_CONFIRM"):
        logger.warning(
            "[permissions] ETSY_AUTO_CONFIRM is deprecated. "
            "Use ETSY_TOOL_PERMISSION_MODE=bypass instead."
        )
