...
Example::
    ...
    from etsy_mcp_shared.diagnostics import init_diagnostics, diagnostics_enabled, wrap_tool

    # During server bootstrap:
    init_diagnostics(
        config_provider=lambda: runtime.config,
        logger_name="etsy-mcp.diagnostics",
    )
"""
...
def init_diagnostics(
    ...
    Args:
        ...
        logger_name: Logger name for diagnostic output (e.g. "etsy-mcp.diagnostics").
    """
...
def _server_diag_cfg_from_env() -> Dict[str, Any]:
    return {
        "enabled": _get_bool_env("ETSY_MCP_DIAGNOSTICS", False),
        "log_tool_args": _get_bool_env("ETSY_MCP_DIAG_LOG_TOOL_ARGS", True),
        "log_tool_result": _get_bool_env("ETSY_MCP_DIAG_LOG_TOOL_RESULT", True),
        "max_payload_chars": _get_int_env("ETSY_MCP_DIAG_MAX_PAYLOAD", 2000),
    }
