...
that all servers (etsy-mcp app) share.
"""
...
def load_server_config(
    *,
    ...
    Then merges server-specific env vars (e.g. ``ETSY_NETWORK_HOST``)
    with fallback to shared vars (e.g. ``ETSY_HOST``).

    Args:
        package_name: Dotted package name for importlib.resources
                      (e.g. ``"etsy_mcp.config"``).
        env_prefix: Server-specific prefix without ``ETSY_`` and trailing ``_``
    ...
    # Merge env vars: server-specific (e.g. ETSY_NETWORK_HOST) > shared (ETSY_HOST)
    etsy_env_overrides: dict[str, Any] = {}
    for key in keys:
        server_key = f"ETSY_{env_prefix}_{key.upper()}"
        shared_key = f"ETSY_{key.upper()}"
        ...
        if val is not None:
            if key == "verify_ssl":
                ...
                val = val.lower()
            etsy_env_overrides[key] = val

    if etsy_env_overrides:
        logger.debug("Applying env overrides to %s config: %s", env_prefix, etsy_env_overrides)
        cfg.etsy = OmegaConf.merge(cfg.etsy, etsy_env_overrides)
...
def validate_registration_mode(logger: logging.Logger) -> str:
    """Read and validate ETSY_TOOL_REGISTRATION_MODE from environment.

    Returns:
        A validated registration mode string ("lazy", "eager", or "meta_only").
    """
    mode = os.getenv("ETSY_TOOL_REGISTRATION_MODE", "lazy").lower()
    if mode not in VALID_REGISTRATION_MODES:
        logger.warning(
            "Invalid ETSY_TOOL_REGISTRATION_MODE: '%s'. Must be one of: %s. Defaulting to 'lazy'.",
            mode,
