...
import pytest
from etsy_mcp_shared.tool_index import (
    TOOL_REGISTRY,
    ToolMetadata,
    get_tool_index,
    register_tool,
...
class TestGetToolIndex:
    ...
    def test_category_filter_with_manifest(self, tmp_path):
        ...
        manifest = {
            "tools": [
                {"name": "etsy_list_clients", "description": "List clients"},
                {"name": "etsy_list_devices", "description": "List devices"},
                {"name": "etsy_block_client", "description": "Block a client"},
            ],
            "module_map": {
                "etsy_list_clients": "app.tools.clients",
                "etsy_list_devices": "app.tools.devices",
                "etsy_block_client": "app.tools.clients",
            },
            "count": 3,
        }
        manifest_path = tmp_path / "tools_manifest.json"
        ...
        assert index["count"] == 2
        names = {t["name"] for t in index["tools"]}
        assert names == {"etsy_list_clients", "etsy_block_client"}
    ...
    def test_category_filter_case_insensitive(self, tmp_path):
        ...
        manifest = {
            "tools": [{"name": "etsy_list_clients", "description": "List"}],
            "module_map": {"etsy_list_clients": "app.tools.clients"},
            "count": 1,
        }
        manifest_path = tmp_path / "tools_manifest.json"
        manifest_path.write_text(json.dumps(manifest))
    ...
    def test_search_matches_name(self):
        register_tool(name="etsy_list_clients", description="List all clients")
        register_tool(name="etsy_list_devices", description="List all devices")
        index = get_tool_index(registration_mode="eager", search="client")
        assert index["count"] == 1
        assert index["tools"][0]["name"] == "etsy_list_clients"
        assert index["filtered"] is True

    def test_search_matches_description(self):
        register_tool(name="etsy_get_stats", description="Get firewall statistics")
        register_tool(name="etsy_list_devices", description="List all devices")
        index = get_tool_index(registration_mode="eager", search="firewall")
        assert index["count"] == 1
        assert index["tools"][0]["name"] == "etsy_get_stats"

    def test_search_case_insensitive(self):
        register_tool(name="etsy_list_clients", description="List clients")
    ...
    def test_category_and_search_combined(self, tmp_path):
        ...
        assert index["count"] == 1
        assert index["tools"][0]["name"] == "etsy_block_client"
